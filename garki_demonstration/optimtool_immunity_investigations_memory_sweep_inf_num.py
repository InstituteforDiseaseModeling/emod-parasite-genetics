...
from immunity_investigations.ImmunityCalibSite import ImmunityCalibSite
...
expname = f'ImmunityCalib_{n_infections}inf_{samples_per_iteration}samps_{max_iterations}iters_test_dtype'
...
def map_sample_to_model_input(cb, sample):
    ...
    for p in params:
        if p['Name'] == 'Falciparum_PfEMP1_Variants':
            value = float(int(sample.pop(p['Name'])))
            tags.update(cb.set_param(p['Name'], value))
        elif p['Name'] == 'Falciparum_Nonspecific_Types':
            value = float(int(sample.pop(p['Name'])))
            tags.update(cb.set_param(p['Name'], value))
