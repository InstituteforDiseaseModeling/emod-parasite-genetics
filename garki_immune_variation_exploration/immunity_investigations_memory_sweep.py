...
from immunity_investigations.var_gene_interventions import add_malaria_outbreak
...
exp_name = f'test_sweep_antigen_memory_and_halflife'
# Setup ----------------------------------------------------------------------------------------------------------
...
infection_number_array = [10]
coverage_array = [1]
period_array = [180]
var_lengths = list(np.logspace(start=2, stop = 4, num= 2))
memory_levels = list(np.linspace(start=0.01,stop = 0.35, num = 2))
hyper_immune_halflives = list(np.linspace(start=10,stop = 360, num = 2))
